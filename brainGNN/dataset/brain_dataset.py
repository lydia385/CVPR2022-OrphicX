...
def ind_val_to_dense(index, edge_attr, size):
    """
    Converts index and edge attribute representations back to a dense adjacency matrix.
    
    Parameters:
    - index: A tensor containing the indices of non-NaN values in the original adjacency matrix.
    - edge_attr: A tensor containing the edge attributes corresponding to the indices.
    - size: The size of the square adjacency matrix (size x size).
    
    Returns:
    - A dense adjacency matrix with NaNs restored in positions that were NaNs in the original matrix.
    """
    adj = torch.full((size, size), float('nan'))
    adj[index] = edge_attr
    return adj
...
class BrainDataset(InMemoryDataset):
    def __init__(self, root, name, transform=None, pre_transform: BaseTransform = None, view=0):
        ...
        adj, _ = load_data_abide(self.raw_dir)
